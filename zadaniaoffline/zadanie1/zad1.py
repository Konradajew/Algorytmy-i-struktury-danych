...
swojej pozycji czyli element minimalny będzię maksymalnie odległy od swojej pozycji o k miejsc, oznacza to, że sortując k+1
...
pierwszych elementów, pierwsze k+1 z nich znajdzie się dokładnie na swoich pozycjach, następnie powtarzamy ten proces przesuwając
...
(n/(k+1))*(2k+1)log(2k+1) < (n/(k+1))*(2k+2)log(2k+1) = (2n+2)log(2k+1) co oznacza, że złożoność ma
postać O(nlogk)
Czyli:
dla k=Θ(1) złożoność ma postać O(n)
dla k=Θ(logn) złożoność ma postać O(nlog(logn))
dla k=Θ(n) złożoność ma postać O(nlogk)
"""
