...
def spacetravel( n, E, S, a, b ):
    ...
    # zbijamy wszystkie punkty w osobliwosci w jeden i bierzemy najkrotsza krawedz
    # rozwiazanie godne prawdziwego psychopaty, ide na wo btw
    # v1
    """
    osob_short_kraw = [float('inf')]*n

    for u, v, p in E:
        if u in S:
            if p < osob_short_kraw[v]:
                osob_short_kraw[v] = p
        elif v in S:
            if p < osob_short_kraw[u]:
                osob_short_kraw[u] = p
        else:
    ...
    for e in S:
        for s, p in neighbours[e]:
            if p < osob_short_kraw[s]:
                osob_short_kraw[s] = p
            #print(e, s, p)
            #print(neighbours)
            neighbours[s].remove((e, p))
            #print(neighbours)
        neighbours[e] = []
    #"""
    for i in range(0, n):
        if osob_short_kraw[i] < float('inf'):
            neighbours[i].append((n, osob_short_kraw[i]))
            neighbours[n].append((i, osob_short_kraw[i]))
    #print(neighbours)
    #optimize(neighbours,a,b)
    return dijkstra(neighbours, a, b, S)
